...
Although  th eclassic  approach for measuring the distance amongtwo  K-dimensional  signal  may  represents  the  best,  insome cases different variants has shown better result [refpaper iDTW, rDTW]. 
MTSS comes with two variants for computing DTW measure, namely:

1. **D-MDTW:** Dependent-Multivariate Dynamic Time Warping
2. **I-MDTW:** Independent-Multivariate Dynamic Time Warping

These two approaches differ from one to another for the way in which the *k's* DTW are computed[@sart2010accelerating]. In addition to these two approaches, our software provides a *Multivariate Rotation Dynamic Time Warping* (R-MDTW) approach, for getting better classification results on specific scenarios.
