...
The *Similarity subseq-search* aim to find the best match for a time series (known as the query) onto another time serie (known as instance). It's achieved by punctually shifting the *instance* time series and comparing it agains the *query* one (See figure below). 
